	...
	this.fetch = function(callback) {		
		...
		connection.on("connect", function() {
			...
			getAdvertisedRefs(pktLineHandler, function(refs, capabilities) {
				...
				console.log(packfileTemp);
				var packfileStream = fs.createWriteStream(packfileTemp.path, {
					fd: packfileTemp.fd
				});
				
				connection.on("close", function() {
					fs.writeFileSync(path.join(repo.path, "..", "testpack.pack"), fs.readFileSync(packfileTemp.path));
				});
				...
				pktLineHandler.getLine(function(line) {
					...
					receiver.on("data", function(data) {
						packfileStream.write(data);
						packfileStream.flush();
